...
function resolvePackageRoot() {
  if (process.env.DINGFENG_HOME) {
    return path.resolve(process.env.DINGFENG_HOME);
  }
  if (app.isPackaged) {
...
function resolveInstanceRoot() {
  if (process.env.DINGFENG_RUNTIME_ROOT) {
    return path.resolve(process.env.DINGFENG_RUNTIME_ROOT);
  }
  if (app.isPackaged) {
  ...
  return new Promise((resolve) => {
    ...
    const request = https.request(
      target.url,
      {
        ...
        timeout: timeoutMs,
        headers: {
          'User-Agent': 'DingFeng-Network-Probe/1.0',
          'Accept': '*/*',
...
function pythonEnv() {
  return {
    ...process.env,
    DINGFENG_RUNTIME_ROOT: INSTANCE_ROOT,
...
function ensureCustomerRuntimeLayout() {
  ...
  const licenseSource = getExistingPath([
    process.env.DINGFENG_LICENSE_PATH,
...
function getEngineExecutable() {
  ...
  const candidates = [
    process.env.DINGFENG_ENGINE_EXECUTABLE,
    path.join(RESOURCES_ROOT, 'engine', 'dingfeng_engine.exe'),
    path.join(RESOURCES_ROOT, 'app.asar.unpacked', 'engine', 'dingfeng_engine.exe'),
    path.join(EXECUTABLE_ROOT, 'resources', 'engine', 'dingfeng_engine.exe'),
    path.join(EXECUTABLE_ROOT, 'app', 'resources', 'engine', 'dingfeng_engine.exe'),
    path.join(EXECUTABLE_ROOT, 'engine', 'dingfeng_engine.exe'),
  ].filter(Boolean);

  return getExistingPath(candidates) || candidates[0] || 'dingfeng_engine.exe';
}

function resolveInstanceId() {
  if (process.env.DINGFENG_INSTANCE_ID) {
    return String(process.env.DINGFENG_INSTANCE_ID);
  }
...
function claimLicenseSlot(status) {
  ...
  const maxInstances = Number.isFinite(requestedMaxInstances)
    ...
    : 1;
  const product = sanitizeSlotPart(status.product || 'DINGFENG');
  ...
  const slotDir = path.join(app.getPath('temp'), 'DingFengSmartFissionV9', 'license-slots', product, machineCode, licenseId);
...
function runtimeMetrics() {
  ...
  const creditCostTotal = Math.trunc(numberValue(status.scrapedo_request_cost_total, 0));
...
ipcMain.handle('runtime:info', async () => {
  ...
  if (!result.data) {
    ...
    result.data.preview_completion_modal = process.env.DINGFENG_PREVIEW_COMPLETION_MODAL === '1';
  }
