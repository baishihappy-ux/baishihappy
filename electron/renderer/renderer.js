...
shipBodyImage.src = './assets/newfeichuan.png';
...
async function copyMachineCode() {
  ...
  await window.dingfeng.copyText(value);
...
async function refreshLog() {
  ...
  const log = await window.dingfeng.tailLog(180);
...
async function refreshResults(kind = 'main') {
  ...
  const result = await window.dingfeng.readResults(kind, 80);
...
async function refreshRuntimeMetrics() {
  if (!window.dingfeng.runtimeMetrics) {
    return null;
  }
  try {
    const metrics = await window.dingfeng.runtimeMetrics();
...
async function testNetworkQuality({ silent = false } = {}) {
  ...
  state.networkTestPromise = window.dingfeng.testNetwork();
  try {
...
async function confirmTargetSource() {
  ...
  const result = await window.dingfeng.setTargetSource(targetSource);
  if (!result.ok) {
...
async function importInputFile() {
  ...
  try {
    const result = await window.dingfeng.importInputFile();
...
async function startEngine(auto = false) {
  ...
  if (window.dingfeng.activeRunLock) {
    const lock = await window.dingfeng.activeRunLock();
    if (lock?.active) {
  ...
  const result = await window.dingfeng.startEngine(state.confirmedTargetSource);
...
async function togglePauseEngine() {
  ...
  try {
    ...
    const result = state.runPaused
      ? await window.dingfeng.resumeEngine()
      : await window.dingfeng.pauseEngine();
...
async function openRuntimeMonitor() {
  if (!window.dingfeng.openRuntimeMonitor) {
    return;
  }
  const result = await window.dingfeng.openRuntimeMonitor();
...
async function refreshDiagnosticModeStatus() {
  if (!window.dingfeng.diagnosticStatus || !$('toggleDiagnosticMode')) {
    return;
  }
  const result = await window.dingfeng.diagnosticStatus();
...
async function resetDiagnosticModeDefaultOff() {
  ...
  updateDiagnosticModeButton();
  if (!window.dingfeng.setDiagnosticEnabled || !$('toggleDiagnosticMode')) {
    return;
  }
  const result = await window.dingfeng.setDiagnosticEnabled(false);
  if (!result.ok) {
...
async function toggleDiagnosticMode() {
  if (!window.dingfeng.setDiagnosticEnabled) {
    return;
  }
  const result = await window.dingfeng.setDiagnosticEnabled(!state.diagnosticModeEnabled);
  if (!result.ok) {
...
async function handleAppCloseRequest() {
  ...
  if (!confirmed) {
    if (window.dingfeng.cancelCloseRequest) {
      await window.dingfeng.cancelCloseRequest();
    }
    return;
  }
  ...
  try {
    await window.dingfeng.gracefulClose();
  } catch (error) {
...
async function startDemo() {
  ...
  const result = await window.dingfeng.startDemo();
...
async function checkLicense() {
  try {
    state.runtime = await window.dingfeng.runtimeInfo();
    ...
    const result = await window.dingfeng.licenseStatus();
...
async function activateLicense(event) {
  ...
  const result = await window.dingfeng.activateLicense(code);
  const status = result.data || {};
...
window.dingfeng.onEngineStdout((value) => {
  const logNode = $('runtimeLog');
  if (logNode) {
...
window.dingfeng.onEngineStderr((value) => {
  const logNode = $('runtimeLog');
  if (logNode) {
    logNode.textContent += value;
    logNode.scrollTop = logNode.scrollHeight;
...
window.dingfeng.onEngineExit((event) => {
  setAnimationSuspended(false);
...
if (window.dingfeng.onAppCloseRequest) {
  window.dingfeng.onAppCloseRequest(handleAppCloseRequest);
}
