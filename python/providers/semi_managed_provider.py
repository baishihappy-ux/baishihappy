...
class SemiManagedHttpProvider(BaseProvider):
    ...
    def fetch(self, task, session=None) -> ProviderResponse:
        ...
        if not self.enable_network:
            html = f"<html><body data-provider='{self.alias}'><a href='{task.url}'>dry run</a></body></html>"
...
class CloudBypassProvider(SemiManagedHttpProvider):
    alias = "cloudbypass"


class ZenRowsProvider(SemiManagedHttpProvider):
    alias = "zenrows"
